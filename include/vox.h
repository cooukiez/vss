...
uint32_t spread_bits(const uint8_t byte) {
    uint32_t x = byte;
    x = (x | (x << 16)) & 0x030000FF;
    x = (x | (x << 8)) & 0x0300F00F;
    x = (x | (x << 4)) & 0x030C30C3;
...
uint32_t morton_encode_3d(const uint8_t x, const uint8_t y, const uint8_t z) {
...
void morton_decode_3d(const uint32_t morton_code, uint8_t &x, uint8_t &y, uint8_t &z) {
